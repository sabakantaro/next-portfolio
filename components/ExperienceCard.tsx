  ...
  return (
    <article className='flex flex-col rounded-lg items-center space-y-4 flex-shrink-0 w-full snap-center bg-[#292929]/60 p-5 hover:opacity-100 opacity-40 cursor-pointer transition-opacity duration-200 overflow-hidden'>
      <motion.img
        ...
        md:w-32 rounded-full object-cover object-center'
      />
      <div className='px-0 md:px-10'>
        <h4 className='text-md md:text-2xl front-light text-green-400'>
          {experience?.jobTitle}
        </h4>
        ...
        <div className='flex space-x-2 my-2 mt-5'>
          {experience.technologies.map((technology) => (
            <img
              ...
              className='h-4 w-4 md:h-10 md:w-10 rounded-full'
            />
          ))}
        </div>
        ...
        <ul className='list-disc text-xs md:text-md max-h-32 overflow-y-scroll'>
          {experience.points.map((point, i) => (
            <li key={i} className='text-xs text-left'>
              - {point}
            </li>
          ))}
        </ul>
